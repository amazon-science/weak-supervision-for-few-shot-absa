...
def _select_k_points_per_class(
    all_sents: List[List[str]],
    all_labels: List[List[Tuple]],
...
def write_subsampled_data(
    output_file: str, all_sents: List[List[str]], all_labels: List[List[Tuple]]
):
...
def get_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help="number of shot for each entity type",
    )
    parser.add_argument(
        "--num_repeat",
        type=int,
        default=1,
        help="number of times to repeat the same sampling strategy",
    )
...
def main():
    ...
    all_sents_original, all_labels_original, unique_labels_original = (
        read_absa_quad_from_file(config.input_file)
    )
    ...
    for i in range(config.num_repeat):
        ...
        subsampled_sents, subsampled_labels, label_count = _select_k_points_per_class(
            all_sents, all_labels, unique_labels, config.num_shot
        )
        output_file = os.path.join(
            config.output_dir,
            os.path.basename(config.input_file).rsplit(".", 1)[0]
            + f"_k_{config.num_shot}_seed_{current_seed}.txt",
        )
        write_subsampled_data(output_file, subsampled_sents, subsampled_labels)
        print(
            f"Iteration={i+1}, k={config.num_shot}, seed={current_seed}, label_count={label_count}"
        )
