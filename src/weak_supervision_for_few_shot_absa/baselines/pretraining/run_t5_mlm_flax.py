...
def main():
    ...
    original_dataset = load_dataset("json", data_files="amazon_yelp.json")[
        "train"
    ].train_test_split(seed=1, test_size=0.2)
    from datasets import DatasetDict

    datasets = DatasetDict()
    datasets["train"] = original_dataset["train"]
    datasets["validation"] = original_dataset["test"]
    ...
    print(data_args.max_seq_length, tokenizer.model_max_length)
    ...
    #     print(tokenizer.decode(model.generate(**tokenizer('Translate the following sentence to French: "This is a very good idea!"', return_tensors='jax'), max_length=128)[0].tolist()[0]))
    #     exit()
    # Data collator
        ...
        for step, batch_idx in enumerate(
            ...
            if cur_step % training_args.save_steps == 0 and cur_step > 0:
                ...
                if jax.process_index() == 0:
                    print(f"Save to {training_args.output_dir}_{cur_step}")
                    params = jax.device_get(jax.tree_map(lambda x: x[0], state.params))
                    model.save_pretrained(
                        f"{training_args.output_dir}_{cur_step}", params=params
                    )
                    tokenizer.save_pretrained(f"{training_args.output_dir}_{cur_step}")
                    if training_args.push_to_hub:
                        repo.push_to_hub(
...
"""
python run_t5_mlm_flax.py --output_dir="./t5-base-absa1"         --dataset_name="NULL" --model_name="t5-base"     --tokenizer_name="t5-base" --per_device_train_batch_size="8" --per_device_eval_batch_size="8" --adafactor --learning_rate="0.005" --weight_decay="0.001" --warmup_steps="500" --overwrite_output_dir --logging_steps="500" --save_steps="500" --eval_steps="500" --max_seq_length 512
"""
